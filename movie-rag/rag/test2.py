...
# โหลดตัวแปรจากไฟล์ .env (เพื่อดึง HUGGINGFACEHUB_API_TOKEN)
load_dotenv()

# --- 2. Imports Library ---
from langchain_community.vectorstores import FAISS
from langchain_huggingface import HuggingFaceEmbeddings
from langchain_huggingface import HuggingFaceEndpoint  # เปลี่ยนจาก OllamaLLM เป็น HuggingFaceEndpoint
...
def load_llm():
    """โหลด LLM ผ่าน Hugging Face Serverless API"""
    print("Loading LLM via Hugging Face API...")
    
    # กำหนดชื่อโมเดลที่ต้องการใช้
    # หาก Llama 3.2 ติดปัญหาเรื่อง Permission (Gated Model) 
    # สามารถเปลี่ยนไปใช้ "mistralai/Mistral-7B-Instruct-v0.3" ได้ครับ
    repo_id = "meta-llama/Llama-3.2-3B-Instruct" 
    
    llm = HuggingFaceEndpoint(
        repo_id=repo_id,
        task="text-generation",
        temperature=0.01,         # ใช้ 0.01 แทน 0.0 สำหรับ HF API
        max_new_tokens=1024,      # ความยาวคำตอบสูงสุด
        do_sample=False,          # ป้องกันการเดาสุ่มคำตอบ (ลด Hallucination)
        return_full_text=False    # สำคัญ! เพื่อไม่ให้ API คืน Prompt กลับมาด้วย
    )
    return llm
...
def answer_question(question, vectorstore, llm, filter_year=None, filter_title=None):
    ...
    print(f"\n[LLM] Generating answer via Hugging Face API...")
